...
if "logged_in" in st.session_state and st.session_state.logged_in:
    ...
    if register.startswith("already"):
        ...
        password_lock = st.text_input("please enter password:", type="password")

        if st.button("sign in"):
            lock = read_data("password", lock_table, "username", username)

            if lock and password_lock == lock[0]:
    ...
    # --- SIGN UP ---
        ...
        password_lock = st.text_input("please enter password(remember for the future logins):", type="password")

        if st.button("Sign Up"):
            if not username or not password_lock:
                ...
                if lock:
                    ...
                    store_data(lock_table, "username", username, password_lock)
